...
BAHAI_WEEKDAYS = ("Jamal", "Kamal", "Fidal", "Idal",
...
def hebrew_delay_1(year):
    ...
    day = floor((months * 29) + parts / 25920)
...
def equinoxe_a_paris(year):
    ...
    #    the Greenwich meridian and that of Paris, 2deg20'15" to the
    #    East.  */
...
def verify(jd, func, args_tuple):
    ...
    if jd != jd_cmp:
        print
        "ERROR: %s(%s) = %s did not match jd (%s)" % (func,
                                                      str(args).
                                                      str(jd_cmp),
                                                      str(jd))
        return 0
